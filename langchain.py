...
class Chatbot:
    ...
    def get_completion_from_messages(self, in_messages, model="gpt-35-turbo", temperature=0):
    ...
    def collect_messages(self, new_message):
        self.chat_history.append({'role':'user', 'content':f"{new_message}"})
        response = self.get_completion_from_messages(self.chat_history) 
        self.chat_history.append({'role':'assistant', 'content':f"{response}"})

        return response
    
    def print_chathistory(self):
...
def test(cb, prompt):
    ...
    print("\n\n\n~~~pre append messages~~~\n\n\n")
    cb.print_chathistory()

    query_body = cb.collect_messages(prompt)
    ...
    for key in clusters:
        ...
        human_readable = cb.get_completion_from_messages(messages2)
...
if input_val:
    ...
    results = test(st.session_state.chatbot, input_val)
    # print(results)
